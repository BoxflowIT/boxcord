...
| Platform | Output |
|---|---|
| Linux | `Boxcord-1.13.0.AppImage`, `boxcord_1.13.0_amd64.deb`, `boxcord-1.13.0-1-x86_64.pkg.tar.zst`, `boxcord-1.13.0.x86_64.rpm` |
...
#### Linux `.deb` installation fallback
...
An `isInstallingUpdate` flag tracks whether the update is in progress so the async `autoUpdater.on('error')` handler can distinguish pkexec failures from unrelated errors.
