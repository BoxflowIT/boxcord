...
import type { SocketHandlerContext, ChannelPayload, ChannelDeletePayload } from '../types';
...
export function registerChannelHandlers(context: SocketHandlerContext): void {
  ...
  socket.on('channel:created', (channel: ChannelPayload) => {
    queryClient.setQueryData<Channel[]>(queryKeys.channels(channel.workspaceId), (old) => {
  ...
  socket.on('channel:deleted', ({ channelId, workspaceId }: ChannelDeletePayload) => {
    queryClient.setQueryData<Channel[]>(queryKeys.channels(workspaceId), (old) => {
      if (!old) return old;
