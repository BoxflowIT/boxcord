...
const presenceHandlers = new Map<string, (data: { userId: string; status: string }) => void>();
...
export function registerPresenceHandlers(context: SocketHandlerContext): void {
  ...
  socket.on('user:online', ({ userId }: { userId: string }) => {
    presenceHandlers.forEach((handler) => handler({ userId, status: 'ONLINE' }));
  });

  // user:offline - User went offline
  socket.on('user:offline', ({ userId }: { userId: string }) => {
    presenceHandlers.forEach((handler) => handler({ userId, status: 'OFFLINE' }));
  });
